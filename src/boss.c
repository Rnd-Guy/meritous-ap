...
SDL_Surface* BossGetSpr (int i)
{
  static SDL_Surface* boss_spr[BOSS_COUNT] = { NULL, NULL, NULL, NULL };
  assert (i>=0 && i<BOSS_COUNT);

  if (boss_spr[i] == NULL) {
    char filename[16];
    sprintf (filename, "dat/i/boss%d.png", i+1);
    boss_spr[i] = IMG_Load(filename);
    if (boss_spr[i] == NULL) {
      fprintf (stderr,
          "Error loading \"%s\": %s\n",
          filename,
...
void InitBossVars()
{
#define PRELOAD_BOSS_SPRITES
#ifdef PRELOAD_BOSS_SPRITES
  // load the boss sprites early, to avoid errors during the game
  int i;
...
void DrawBoss()
{
  SDL_Surface* boss_spr = BossGetSpr (current_boss);
  int flash_coeff;
  static int t = 0;
  t++;

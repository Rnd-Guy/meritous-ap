...
void CurseSingleEnemy(struct enemy *e)
{
	...
	// only place curse enemies in normal or post-special rooms
	if (NActiveRooms == 0) {
		for (i = 0; i < rooms_to_gen; i++) {
	...
	// try to find a room we've visited that doesn't already have 2 enemies.
	// limit the attempts because this may be impossible if cursed seal was picked up early and loop infinitely
